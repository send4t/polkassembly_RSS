export const ALICE = "12dZDawZByadbVmjBUv96M55KbZRLsbnRdfjrhmHtSFvkh48";
export const BOB = "16GK2GsTbNHaV9nLevGU2C1bzb4mDihhdmk2kfEQjd4SWYnR";
export const CHARLOTTE = "15m9QSgHN92SnGvmmvin83NW9VKn8GUNmqPwGR6hyHKUaMqV";

const MULTISIG = [ALICE, BOB, CHARLOTTE];

export const SS58_FORMAT = 0;

export const APP_NAME = 'HappyApp';

export const PASEO_PROVIDER = 'wss://paseo.rpc.amforc.com';
export const KUSAMA_PROVIDER = 'wss://kusama-rpc.polkadot.io'//'wss://rpc.ibp.network/kusama';

export const BALANCE = 1000000000; // 1 zero was removed

export const THRESHOLD = 2;

export const MIMIR_URL = 'https://mimir-client.mimir.global';