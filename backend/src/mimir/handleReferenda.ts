...
export async function handleReferendaVote(page: NotionPage, network: Chain, postId: ReferendumId, readyProposals: ReadyProposal[]): Promise<void> {
    let multisig: string = "";
    ...
    if (page.properties?.['Internal status'].status?.name === InternalStatus.ReadyToVote) {
        ...
        switch (page.properties?.['Suggested vote'].select?.name) {
            case SuggestedVote.Aye:
                ...
                ready = await proposeVoteTransaction(multisig, network, postId, SuggestedVote.Aye);
                break;
            case SuggestedVote.Nay:
                console.info("Sending transaction to Mimir (Nay) ...");
                ready = await proposeVoteTransaction(multisig, network, postId, SuggestedVote.Nay);
                break;
            case SuggestedVote.Abstain:
                console.info("Sending transaction to Mimir (Abstain) ...");
                ready = await proposeVoteTransaction(multisig, network, postId, SuggestedVote.Abstain);
                break;
            default:
