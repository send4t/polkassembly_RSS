...
async function fetchReferendums() {
    try {
        const response = await fetch('https://api.polkassembly.io/api/v1/latest-activity/all-posts?govType=open_gov&listingLimit=50', {
            method: 'GET',
            headers: {
                'x-network': 'polkadot', 
        ...
        // Filter posts created in the last 28 days
        const recentPosts = data.posts.filter(post => {
            const postDate = new Date(post.created_at);
            return postDate >= twentyEightDaysAgo && postDate <= now;
        });
